...
class ASTParser:
    ...
    def _parse_tree_sitter(
        ...
        _NAME_TYPES = ("identifier", "name", "field_identifier", "property_identifier")

        def _text(child: Any) -> str:
        ...
        def node_name(node: Any) -> str:
            for child in node.children:
                if child.type in _NAME_TYPES:
                    return _text(child)
            # Anonymous (e.g. arrow function) — borrow the name it's assigned to:
            #   const foo = () => …   |   foo: () => …   |   foo = () => …
            parent = node.parent
            if parent and parent.type in (
                ...
                for child in parent.children:
                    if child.type in _NAME_TYPES + ("shorthand_property_identifier",):
                        return _text(child)
            return "<anonymous>"
